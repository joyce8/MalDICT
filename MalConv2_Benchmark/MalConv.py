...
def getParams():
    params = {
        'channels': ("suggest_int", {'name':'channels', 'low':32,
                                     'high':1024}),
        'log_stride': ("suggest_int", {'name':'log2_stride', 'low':2,
                                       'high':9}),
        'window_size': ("suggest_int", {'name':'window_size', 'low':32,
                                        'high':512}),
...
class MalConv(LowMemConvBase):
    ...
    def __init__(self, out_size=2, channels=128, window_size=512, stride=512,
